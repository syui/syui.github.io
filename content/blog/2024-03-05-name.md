...
例えば、私の本名が`一ノ瀬 修(いちのせ しゅう)`ならこうなります。なお、当該日本名はペンネームということにしておきます(作品書いてないけど)。
...
> この記事はblueskyで1年を過ごして思ったことなどを加筆することにしました。

私がblueskyを始めて1年が過ぎました。アカウントは[@syui.ai](https://bsky.syui.ai/)です。

blueskyを始めた日付はplcに記録されています。初投稿はpdsにあります。
...
```sh
$ plc=`curl -sL "https://plc.directory/export?after=2023-03-19"|jq "select(.operation.services.atproto_pds.endpoint == \"https://syui.ai:2583\")"|jq -s ".[0]"`
...
> ただし、plcの負担になるのでexportの結果をlocalに保存し使い回すようにしてください。ある程度のcool-timeを置くようにしてください。
...
<div class="sketchfab-embed-wrapper"> <iframe title="super" frameborder="0" allowfullscreen mozallowfullscreen="true" webkitallowfullscreen="true" allow="autoplay; fullscreen; xr-spatial-tracking" xr-spatial-tracking execution-while-out-of-viewport execution-while-not-rendered web-share width="100%" height="480" src="https://sketchfab.com/models/9a80a6d6cf6f4b08906505c7f945d3ce/embed?autospin=1&autostart=1"> </iframe> </div>

`aibot`は、rustで書かれており、`aios`という自作osの上で動いています。
...
repositoryは`ai/os`みたいな形式にすることでusernameから`ai`を補完しています。repositoryが増えるたび`aixxx`になり検索性、可読性に欠けるためです。
...
ちなみに、ghcr.ioの名前は、おそらくgh-crに分解できます。つまり、`[g]it[h]ub-[c]ontainer-[r]egistry`だと思われます。`io`はgithubが好んで使うdomainです。web-serverのgithub.ioにも使われています。
...
例えば、[ai/os](https://git.syui.ai/ai/os)も名前だけはちょっとだけ面白いんだよ。
