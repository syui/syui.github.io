...
読み込むと`Animation Clip`ができます。これはunity独自のmodel motionのようなものです。まずはfbxの`Animation Type`を`Humanoid`にします。そして、`Amimation Clip`を右クリックして、`create -> Animator Controlle`を作成し、`Entry -> State`を作成したうえで、`motion`にfbxのmotionを当てます。ここではue5から持ってきたmotionなので、`Unreal Take`になります。

> ue5からfbxをexportする際は、animationですべてのチェックを付けましょう。精度が高まります。あと、コリジョンは外しました。

- 1. Animation Type : Humanoid
- 2. Create : Animator Controlle
- 3. Entry : New State
- 4. Animation Clip -> .vrma
