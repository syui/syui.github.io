...
ここで、entの通常の開発手順としては、(1)`ent/schema/entry.go`を編集する、(2)`go generate ./ent`を実行する、(3)`main.go`などのメインファイルからclientを操作する、(4)`go run`や`go build`する、という流れになります。
...
project-rootから`go generate ./ent`を実行します。すると、先程、initした名前のファイル、`entry*.go`が作成されています。これらをimportすることで、開発者は`scheme`と`main`の編集に集中できるという感じになります。
...
できました。以上がentでopen-apiを作成する基本的な手順になると思われます。
