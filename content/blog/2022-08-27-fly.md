...
herokuがplan:free(hobby)を廃止し、`m/$31`になったので、[heroku](https://heroku.com/)から[fly.io](https://fly.io/)へ移行を考えています。
...
`OTP_SECRET`, `SECRET_KEY_BASE`は`fly secrets set`したほうがいいです。
...
$ fly volumes create -c fly.redis.toml xxx_redis --size 1
...
```sh
$ fly volumes create xxx_uploads --size 1
```
...
herokuに比べてすごく遅いです。sidekiqが遅く、redisが原因だと思われます。しかし、これ以上の方法があるとは思えません。
...
fly-postgres(pg)はpublicではなくprivateです。container内からしかアクセスできません。ただし、localからアクセスできるようになっています。

heroku-pgのdumpをfly-pgに追加します。
