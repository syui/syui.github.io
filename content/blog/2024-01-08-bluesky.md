...
|bsky|SERVICE_SIGNING_KEY|`$ openssl ecparam --name secp256k1 --genkey --noout --outform DER | tail --bytes=+8 | head --bytes=32 | xxd --plain --cols 32`|
...
> `SERVER_DID=did:web:xxx`の場合はローカル環境で生成したもので構いません。しかし、`SERVER_DID=did:plc:xxx`を使用する場合はplcからkeyを登録します。
